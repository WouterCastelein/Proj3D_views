...
def metric_trustworthiness(X_high, X_low, D_high, D_low, k=7):
...
def metric_continuity(X_high, X_low, D_high, D_low, k=7):
    # D_high = compute_distance_matrix(X_high)
    # D_low = compute_distance_matrix(X_low)
...
def metric_shepard_diagram_correlation(X_high, X_low, D_high, D_low):
    # D_high = compute_distance_list(X_high)
    # D_low = compute_distance_list(X_low)

    return stats.spearmanr(D_high, D_low)[0]

def metric_normalized_stress(X_high, X_low, D_high, D_low):
    # D_high = compute_distance_list(X_high)
    # D_low = compute_distance_list(X_low)

    return 1 - np.sum((D_high - D_low)**2) / np.sum(D_high**2)
