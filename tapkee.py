...
class TapkeeCLIProjection(BaseEstimator, TransformerMixin):
    def __init__(self, n_components, projection, command, verbose):
    ...
    def _run(self, X, y, cmdargs):
        ...
        cmdline = [self.command, '--target-dimension', str(self.n_components), '--method', self.projection, '--eigen-method', 'dense', '--neighbors-method', 'brute',
...
class DiffusionMaps(TapkeeCLIProjection):
    def __init__(self, n_components, command=os.getcwd() + '/tapkee/tapkee', t=2, width=1.0, verbose=False):
        super(DiffusionMaps, self).__init__(
            n_components=n_components, projection='diffusion_map', command=command, verbose=verbose)
...
class StochasticProximityEmbedding(TapkeeCLIProjection):
    def __init__(self, n_components, command=os.getcwd() + '/tapkee/tapkee', n_neighbors=12, n_updates=20, max_iter=0, verbose=False):
        super(StochasticProximityEmbedding, self).__init__(
            n_components=n_components, projection='stochastic_proximity_embedding', command=command, verbose=verbose)
...
class LocalityPreservingProjections(TapkeeCLIProjection):
    def __init__(self, n_components, command=os.getcwd() + '/tapkee/tapkee', n_neighbors=10, verbose=False):
        super(LocalityPreservingProjections, self).__init__(
            n_components=n_components, projection='locality_preserving_projections', command=command, verbose=verbose)
...
class LinearLocalTangentSpaceAlignment(TapkeeCLIProjection):
    def __init__(self, n_components, command=os.getcwd() + '/tapkee/tapkee', n_neighbors=10, verbose=False):
        super(LinearLocalTangentSpaceAlignment, self).__init__(
            n_components=n_components, projection='linear_local_tangent_space_alignment', command=command, verbose=verbose)
        self.set_params(command, n_neighbors, verbose)
...
class NeighborhoodPreservingEmbedding(TapkeeCLIProjection):
    def __init__(self, n_components, command=os.getcwd() + '/tapkee/tapkee', n_neighbors=10, verbose=False):
        super(NeighborhoodPreservingEmbedding, self).__init__(
            n_components=n_components, projection='neighborhood_preserving_embedding', command=command, verbose=verbose)
        self.set_params(command, n_neighbors, verbose)
...
class LandmarkMDS(TapkeeCLIProjection):
    def __init__(self, n_components, command=os.getcwd() + '/tapkee/tapkee', n_neighbors=10, verbose=False):
        super(LandmarkMDS, self).__init__(
            n_components=n_components, projection='landmark_multidimensional_scaling', command=command, verbose=verbose)
        self.set_params(command, n_neighbors, verbose)
